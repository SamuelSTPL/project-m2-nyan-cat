...
class Player {
  ...
  constructor(root) {
    ...
    this.domElement.src = `images/bikeTron${backgroundColor}.png`;
    this.domElement.style.maxWidth = PLAYER_WIDTH;
    this.domElement.style.minHeight = PLAYER_HEIGHT;
  ...
  update(color) {
    this.domElement.src = `images/bikeTron${color}.png`;
  }
  // console.log(update())
  ...
  moveRight() {
    if (this.x + PLAYER_WIDTH < GAME_WIDTH + 470) {
      this.x = this.x + PLAYER_WIDTH / 2;
    }
    this.domElement.style.left = `${this.x}px`;
  }
}
// console.log(this.domElement.update());
